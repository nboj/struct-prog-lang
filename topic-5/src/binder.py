from .parser import Program, IfStmt, Expr, Stmt, LetStmt, Binary, Unary, Variable, CallExpr, Literal, Grouping, Assign, ExprStmt, BlockStmt, WhileStmt
...
class Binder:
    ...
    def bind_stmt(self, stmt: Stmt):
        if isinstance(stmt, IfStmt):
            ...
            condition = self.bind_expression(stmt.condition)
            return BoundWhileStmt(condition=condition, block=self.bind_block(stmt.block), span=stmt.span)
        else:
