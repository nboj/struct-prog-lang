...
class Parser:
            ...
            if self.at(TokenType.OpenParen):
                ...
                if not self.at(TokenType.CloseParen):
                    ...
                    while self.at(TokenType.Comma):
                        _ = self.advance()
                        args.append(self.parse_expr())
                r = self.expect(TokenType.CloseParen, "expected )")  # right paren
                primary = CallExpr(
    ...
    def parse_primary(self):
        ...
        match tok.kind:
            ...
            case _:
                raise AssertionError(self.err_here(tok, f"unexpected token {tok.kind}"))
        ...
        match token.kind:
            ...
            case TokenType.Let:
                ...
                if not isinstance(assign, Assign):
                    raise AssertionError(
                        self.to_err(assign, f"incorrect lhs node: {assign}")
                    )
                if not isinstance(assign.target, Variable):
                    raise AssertionError(
                        self.to_err(assign, "invalid lhs in assignment")
                    )
            ...
            case TokenType.If:
                ...
                while token.kind != TokenType.CloseCurly:
                    ...
                    if token.kind == TokenType.Eof:
                        raise AssertionError(
                            self.err_here(token, "reached EOF before closing")
                        )
                    self.consume_terminators()
                _ = self.advance()
            ...
            case TokenType.While:
                ...
                if self.at(TokenType.Eof):
                    raise AssertionError(self.err_here(self.peek(), "expected }"))
    ...
    def err_here(self, tok: Token, msg: str) -> str:
        (sline, scol), (_eline, ecol) = self.sm.span_to_lc(tok.span)
    ...
    def to_err(self, node: Node, msg: str) -> str:
        ...
        header = (
            f"At {sline}:{
            scol}"
            + (f"-{eline}:{ecol}" if (sline, scol) != (eline, ecol) else "")
        )
        parts.append(header)
        ...
        if sline == eline:
            ...
            first_carets = " " * (scol - 1) + "^" * max(
                1, max(0, len(first) - (scol - 1))
            )
